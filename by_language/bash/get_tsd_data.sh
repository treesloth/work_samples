...
tsdb_datadir=$basedir/$tsdb_databasedir;    mkdir -p $tsdb_datadir
echo "Data will be written to $tsdb_datadir"
...
tsdb_dates() {
    # Generate a TSD-compatible start and end date and a datastamp appropriate for a filename
    i=$1
...
baseurl="https://tsdb_.monilytics.net/metrics-archive/tsdb_/query2x?"
...
for i in $(eval echo {1..$periods}); do
    for metric in ${metrics[@]}; do
        read startdt enddt maintepoch <<< $(tsdb_dates $i)
    ...
    #    curl -gsk $qurl | tail -n +2 >> data/$datafile 
        ...
        if (( "$HTTP_STAT" != "200" )); then
            ...
            touch $tsdb_datadir/$datafile
            echo "Writing to" $tsdb_datadir/$datafile
            ls -al $tsdb_datadir/$datafile
            tail -n +$startline $run_tempfile >> $tsdb_datadir/$datafile
        fi
...
for file in "${!datafilearr[@]}"; do
    ...
    dos2unix $tsdb_datadir/$file
    sort $tsdb_datadir/$file | uniq > $tsdb_datadir/tmpfile
    mv $tsdb_datadir/tmpfile $tsdb_datadir/$file
    if [[ -e $tsdb_datadir/$file ]]; then
        rm $tsdb_datadir/$file
    fi
done
